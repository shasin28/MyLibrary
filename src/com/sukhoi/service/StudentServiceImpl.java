...
public class StudentServiceImpl implements StudentService {
	...
	public boolean removeStudent(int studentId) throws StudentIdNotFoundException {
		...
		if (student != null) {
			...
			throw new StudentIdNotFoundException(
					"Seems this student id is from an alien planet ; not registered in the library at present ");
		}
	}
	...
	public void studentStatusView(int studentId, boolean admin) throws StudentIdNotFoundException {
		...
		if (currentStudent != null) {
			...
			System.out.println(
					"Number of maximum book we allow to be issued at a time: " + StudentService.maxmNoOfIssuedBooks);
			System.out.println("Books have to be returned within " + StudentService.maxmDaysWithNoFine
					+ " days ; after that we charge fine ");

			System.out
					.println("*****************" + name + (admin ? "'s" : "r") + " Library Karma********************");
					...
					try {
						...
						if (book != null) {
							if (book.isAvailability()) {
								...
								return bookDao.updateBookStatus(issueBookId, false, studentId, currIssueDate,
										currReturnDate);
							} else {
								System.out.println("Sorry the given book is already issued to geek with student id "
										+ book.getStudentId());
							}
						} else
						...
						System.out.println(
								"Meditate a bit and put the book id more carefully; better luck ! Found Illegal book id format");
					}

				} else {
					System.out.println(
							"Nevermind try getting any other book! we have abundant books with ocean of knowledge ;)");
				}
			} else {
	...
	public boolean ReturnBook(int studentId) throws StudentIdNotFoundException, BookNotFoundException {
		...
		if (student != null) {
			...
			try {
				...
				System.out.println(
						"Meditate a bit and put the student id more carefully; better luck ! Found Illegal student id format");
			}
		} else
			throw new StudentIdNotFoundException("This geek is from an alien planet;  not registered!! ");
